...
def ensure_member(trip, user):
    if not TripMember.objects.filter(trip=trip, user=user, is_active=True).exists():
        raise serializers.ValidationError("Only trip members can perform this action.")
...
def remove_member(trip, actor, member):
    ...
    Notification.objects.create(
        user=member.user,
        actor=actor,
        type=Notification.Type.REMOVED_FROM_TRIP,
        title="Removed from trip",
        message=f"You were removed from {trip.name}.",
    )
    return member
...
def transfer_ownership(trip, actor, new_owner):
    if trip.owner != actor:
        raise serializers.ValidationError("Only the owner can transfer ownership.")
    ...
    membership.role = TripMember.Role.OWNER
    membership.save(update_fields=["role"])
    trip.owner = new_owner
    trip.save(update_fields=["owner", "updated_at"])
    return trip
