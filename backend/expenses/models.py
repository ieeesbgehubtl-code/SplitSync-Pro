...
class ExpenseParticipant(models.Model):
    ...
    settlement_status = models.CharField(
        max_length=20, choices=SettlementStatus.choices, default=SettlementStatus.OPEN
    )

    class Meta:
        unique_together = [("expense", "user")]
        indexes = [models.Index(fields=["user", "settlement_status"])]
...
class ExpenseAudit(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    expense = models.ForeignKey(
        Expense, on_delete=models.CASCADE, related_name="audit_entries"
    )
